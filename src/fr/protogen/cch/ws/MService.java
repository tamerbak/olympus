...
public class MService {
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public Response selectApplication(InputStream is){
		...
		for(CCHUser u : GUserSession.getInstance().getOnlineUsers())
			if(u.getUid().equals(app.getUser().getUid())){
				user = u;
				break;
			}
		...
		DAL dal = new DAL();
		List<SApplication> applications = dal.selectAllApplications();
		SApplication selectedApp  = null;
		for(SApplication a : applications)
			if(a.getAppKey().equals(app.getAppKey()))
			{
				selectedApp = a;
				break;
			}
	...
	public Response chargerApplication(InputStream is){
		...
		CCHUser usr = new JSONDeserializer<CCHUser>().deserialize(jsonText);
		CCHUser user = null;
		String res = "{status:false}";
		for(CCHUser u : GUserSession.getInstance().getOnlineUsers())
			if(u.getUid().equals(usr.getUid())){
				user = u;
				break;
			}
		...
		res = new JSONSerializer().serialize(application);
		return Response.status(200).entity(res).build();
	}
	...
	@Consumes(MediaType.APPLICATION_JSON + ";charset=utf-8")
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public Response mlajApplication(InputStream is){
		StringBuilder jsBuilder = new StringBuilder();
		try {
			BufferedReader in = new BufferedReader(new InputStreamReader(is, "UTF-8"));
			String line = null;
		...
		SApplication application = session.getApplication();
		application.setAuthor(app.getAuthor());
		application.setProjectName(app.getProjectName());
		application.setLicence(app.getLicence());
